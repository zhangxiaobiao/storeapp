    ...
    public function login()
    {
        ...
        if ($findThirdPartyUser) {
            ...
            Db::name("third_party_user")
                ...
                ->where('app_id', $appId)
                ->update($userData);
//            $userId = Db::name('third_party_user')
//                ->where('openid', $openid)
//                ->where('app_id', $appId)
//                ->field('user_id')
//                ->find();
            Db::name('user_token')->where('user_id',$findThirdPartyUser['user_id'])->update(['token'=>$token]);

//            $bindNum = Db::name('user')
//                ->where('id', $userId['user_id'])
//                ->field('mobile')
//                ->find();
            $bindNum = Db::name('user_token')->where('user_id', $findThirdPartyUser['user_id'])->field('actoken')->find();
//            if ($bindNum['mobile'] == ''){
//                $actoken = '';
//            } else {
//                $actoken = md5($bindNum['mobile']);
//            }
            if ($bindNum['actoken'] == ''){
                $this->actoken = '';
            } else {
                $this->actoken = $bindNum['actoken'];
            }
        } else {
        ...
        $this->success("登录成功!", ['token' => $token, 'actoken' => $this->actoken]);
    }
